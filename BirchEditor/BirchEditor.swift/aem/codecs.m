...
- (NSAppleEventDescriptor *)pack:(id)anObject {
	...
	if ([anObject conformsToProtocol: @protocol(AEMSelfPackingProtocol)]) // AEM application, Boolean, file, type, specifier objects
		...
		if (strcmp([anObject objCType], @encode(BOOL)) == 0) { // JESSE
			return [NSAppleEventDescriptor descriptorWithBoolean:[anObject boolValue]];
		}
		...
		switch (*[anObject objCType]) {
			...
			case 'q':
                ...
                if (sint64 >= /* DISABLES CODE */ (0x80000000) && sint64 < 0x7FFFFFFF)
						goto packAsSInt32;
		...
		result = [NSAppleEventDescriptor descriptorWithTypeCode:'null']; // JESSE
...
- (id)unpack:(NSAppleEventDescriptor *)desc {
	...
	switch ([desc descriptorType]) {
		...
		case typeUnicodeText:
			...
			break;
		case typeFalse:
			result = [NSNumber numberWithBool:NO]; // JESSE
			//result = ASFalse;
			break;
		case typeTrue:
			result = [NSNumber numberWithBool:YES]; // JESSE
			//result = ASTrue;
			break;
...
- (id)unpackType:(NSAppleEventDescriptor *)desc {
	if ([desc typeCodeValue] == 'null') {
		return [NSNull null]; // JESSE
	}
...
- (id)unpackContainsCompDescriptorWithOperand1:(id)op1 operand2:(id)op2 {
	if ([op1 isKindOfClass: [AEMQuery class]] && [[op1 root] isEqualTo: AEMIts])
		return [(AEMResolver *)op1 contains: op2];
	else
		return [op2 isIn: op1];
}
