...
describe 'ItemPath', ->
  ...
  describe 'Predicate', ->
    ...
    describe 'Relations', ->
      ...
      it 'should support = with empty value on item text', ->
        one.bodyString = ''
        outline.evaluateItemPath('//= ""').should.eql [one]

      it 'should support = with empty value on tag text', ->
        one.bodyString = ''
        outline.evaluateItemPath('//@t = ""').should.eql [three, four]
    ...
    describe 'Optional Options', ->
      ...
      it 'should support convert to boolean before compare option', ->
        one.bodyString = 'true'
        outline.evaluateItemPath('@text =[b] true').should.eql([one])
      ...
      it 'should support convert to array before compare option', ->
        one.bodyString = 'a, b, c'
        outline.evaluateItemPath('@text = a,b , c').should.eql([])
        outline.evaluateItemPath('@text = [l] a,b , c').should.eql([one])
        outline.evaluateItemPath('@text beginswith [l] a,b').should.eql([one])
        outline.evaluateItemPath('@text contains [l] b').should.eql([one])
        outline.evaluateItemPath('@text contains [l] c,a').should.eql([one])
        outline.evaluateItemPath('@text endswith [l] b,c').should.eql([one])

      it 'should support convert to array of numbers before compare option', ->
        one.bodyString = '0.1, 20    , 07.0'
        outline.evaluateItemPath('@text = [nl] 0.1,020,7').should.eql([one])
        one.bodyString = '0.1, 20    , 07.0'
        outline.evaluateItemPath('@text < [nl] 0.11').should.eql([one])
        outline.evaluateItemPath('@text < [nl] 0.1').should.eql([])
        outline.evaluateItemPath('@text < [nl] 0.11,0').should.eql([])

      it 'should support convert to array of regex before compare option', ->
        one.bodyString = '0.1, 20    , 07.0'
        outline.evaluateItemPath('@text matches[l] 7').should.eql([one])
        outline.evaluateItemPath('@text matches[l] ^7').should.eql([])
        outline.evaluateItemPath('@text matches[l] ^2,7').should.eql([one])
  ...
  describe 'To String', ->
    ...
    it 'should convert path object to path string', ->
      ...
      new ItemPath('= [sl] insensitive').toString().should.equal('= [sl] insensitive')
