...
describe 'Item', ->
  ...
  it 'should insert item and match next siblings indent', ->
    six.indent = 2
    six.indent.should.equal(2)
    newItem = outline.createItem('new')
    five.insertChildrenBefore(newItem, six)
    newItem.indent.should.equal(2)

  it 'should insert multiple items and match next siblings indent', ->
    six.indent = 2
    six.indent.should.equal(2)
    newItems = [outline.createItem('new'), outline.createItem('new')]
    five.insertChildrenBefore(newItems, six)
    newItems[0].indent.should.equal(2)
    newItems[1].indent.should.equal(2)

  it 'should insert multiple items and match prev siblings indent if no next sibling', ->
    six.indent = 2
    six.indent.should.equal(2)
    newItems = [outline.createItem('new'), outline.createItem('new')]
    five.appendChildren(newItems)
    newItems[0].indent.should.equal(2)
    newItems[1].indent.should.equal(2)

  it 'should insert multiple overindented items', ->
    six.removeFromParent()
    newItems = [outline.createItem('new'), outline.createItem('new')]
    newItems[0].indent = 2
    newItems[1].indent = 2
    five.appendChildren(newItems)
    newItems[0].indent.should.equal(1)
    newItems[1].indent.should.equal(1)
  ...
  describe 'Attributes', ->
    ...
    it 'should get/set boolean attributes', ->
      five.setAttribute('test', true)
      five.getAttribute('test').should.eql('true')
      five.getAttribute('test', Boolean).should.eql(true)
    ...
    it 'should get/set date attributes', ->
      ...
      date = new Date(1976, 11, 27, 0, 0, 0, 1)
      five.setAttribute('test', date)
      five.getAttribute('test').should.eql('1976-12-27T05:00:00.001Z')
      five.getAttribute('test', Date).should.eql(date)
