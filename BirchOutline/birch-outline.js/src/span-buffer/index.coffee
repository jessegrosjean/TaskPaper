...
class SpanBuffer extends SpanBranch
  ...
  getSpanRangeForYOffsetRange: (topYOffset, bottomYOffset) ->
    if @getSpanCount() is 0
      return {} =
        location: 0
        length: 0
    start = @getSpanInfoAtYOffset(topYOffset)
    end = @getSpanInfoAtYOffset(bottomYOffset)
    if end.location is 0 and end.spanIndex isnt start.spanIndex
      end.spanIndex--
    {} =
      location: start.spanIndex
  ...
  insertSpans: (spanIndex, spans) ->
    ...
    if @emitter
      if @scheduledChangeEvent
        @scheduledChangeEvent.insertedSpans.concat(spans)
      else
  ...
  removeSpans: (spanIndex, removeCount) ->
    ...
    if @emitter
      if @scheduledChangeEvent
        @scheduledChangeEvent.removedSpans.concat(@getSpans(spanIndex, removeCount))
      else
        replacedLength = 0
        removedSpans = []
