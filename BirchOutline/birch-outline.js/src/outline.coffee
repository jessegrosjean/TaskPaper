...
class Outline
  ...
  @createWriteRoomOutline: (content) ->
    new Outline(ItemSerializer.WriteRoomType, content)
  ...
  getAttributeNamesToValues: (autoIncludeAttributes=[], excludeAttributes=[]) ->
    ...
    for each in @root.descendants
      for eachAttributeName in Object.keys(each.attributes)
        if excludeAttributes.indexOf(eachAttributeName) is -1
          valuesSet = namesToValues.get(eachAttributeName) ? new Set()
          for eachValue in each.getAttribute(eachAttributeName, undefined, true)
  ...
  getTagAttributeNamesToValues: (autoIncludeAttributes=[], excludeAttributes=[]) ->
    @getAttributeNamesToValues(autoIncludeAttributes, excludeAttributes).filter (each) ->
      each[0].substring(0, 5) is 'data-'
  ...
  recordChange: (mutation) ->
    ...
    if @coalescingMutation and @coalescingMutation.coalesce(mutation)
      ...
      if undoSelection and @coalescingMutation.type is Mutation.BODY_CHANGED
        ...
        undoSelection.headOffset = @coalescingMutation.insertedTextLocation + @coalescingMutation.replacedTextLength
        undoSelection.endOffset = @coalescingMutation.insertedTextLocation + @coalescingMutation.replacedTextLength
    else
  ...
  deserializeItems: (serializedItems, options={}) ->
    try
      return ItemSerializer.deserializeItems(serializedItems, @, options)
    catch e
      return null
